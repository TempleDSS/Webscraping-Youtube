---
title: "Digital_Historian Youtube Comment Mining"
author: "Jeff Antsen"
date: "5/12/2020"
output:
...
##                     __       rm(list=ls())
```
### We should make a chunk where you can set options (tolerance, WF converge, others? (dir. prob not necessary))
```{r Settings_and_tolerances}
# build this up, as/if necessary

```


### A new Function to create Data Frame FROM CSVs of selected comments (update desired columns to keep more!!)
```{r DF_function_unneededButKeptForNow, echo=FALSE}
###############################################################
...
colnames(com.array) <- com.colnames  #++++++++++++++++++++++++++#@ storage list for final data output
  for(f in 1:length(files.v)){
    ...
    tab.df <- NA
    tab.df <- read.csv(paste(data.dir, input.dir, files.v[f], sep=""), 
                         comment.char='#', stringsAsFactors=FALSE)
                             ####OLD#    text.v <- scan(paste(input.dir, files.v[i], sep="/"), what="character", sep="\n")
        for(r in 1:nrow(tab.df)){
            sto <- NA; user <- NA
            sto <- tab.df$textOriginal[r]
            user <- tab.df$authorDisplayName[r]
...
```{r practiceSpace}
## was testing bits of code for the function
...
## NEW
subject.s <- "COVID_??"  #@ what the project is about
...
```{r old_read_data, echo=F, results='hide'}
...
```{r old_loop_n_stuff, echo=F, eval=F}
#@ Make sure you are selecting video IDs that exist at this moment
#@ if the next part of this code throws a HTTP failure: 400, 
#@ a video may have been taken down
#@ some of these HTTP failure: 400 errors may be transient (an error with the YT API)
#@ and you may be able to scrape comments for that video at another time


############ OLD LOOP for collecting comments
#create data frame to hold comments for all videos
#complete_comments <- data.frame()

#for loop to iteratively collect for multiple videos
#creates list to hold results (complete_comments)
#calls get_all_comments function for each video and appends results
...
#for (i in video_list) {
#  more_comments <- get_all_comments(video_id = i)
#  even_more_comments <- data.frame(more_comments)
#  print(i)
#  complete_comments <- smartbind(complete_comments, even_more_comments)
#}



nrow(complete_comments)
View(complete_comments)

#video_1_com <- get_all_comments(video_id = video_list[1])
nrow(video_1_com)
#video_2_com <- get_all_comments(video_id = video_list[2])
nrow(video_2_com)
```
...
app_id <- "1039670268805-fkttcaq01d0qocb9bl9dcc284r1cp565.apps.googleusercontent.com"
app_secret <- "CZ5a8nuGKmr7BtM-BOp9a7Eh"
